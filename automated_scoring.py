...
def calculate_style_score() -> Tuple[float, str]:
    ...
    score = max(0.0, 1 - (violation_count / 100))  # Deduct 1% per violation up to 100
...
def calculate_type_check_score() -> Tuple[float, str]:
    ...
    score = max(0.0, 1 - (error_count / 50))  # Deduct 2% per error up to 50
...
def calculate_formatting_score() -> Tuple[float, str]:
    ...
    score = max(0.0, 1 - (files_count / 10))  # Deduct 10% per file up to 10 files
