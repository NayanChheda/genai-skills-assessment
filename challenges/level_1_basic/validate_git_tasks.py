...
def analyze_commit_messages() -> Tuple[float, str]:
    ...
    score = (
        (meaningful_commits / total_commits) * 0.6
        + (verb_commits / total_commits) * 0.3
        + (min(avg_length, 60) / 60) * 0.1
    )

    details = (
        f"Meaningful commits: {meaningful_commits}/{total_commits}, "
        f"Action-oriented: {verb_commits}/{total_commits}"
    )
...
def check_branch_structure() -> Tuple[float, str]:
    ...
    details = (
        ...
        f"Has main: {has_main_branch}, "
        f"Has features: {has_feature_branches}"
    )
    return round(score, 2), details
