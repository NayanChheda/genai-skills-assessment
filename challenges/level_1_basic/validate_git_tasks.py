...
def analyze_commit_messages():
    ...
    if total_commits == 0:
        return 0, "No commits to analyze"
    ...
    score = (
        (meaningful_commits / total_commits)
        * 0.6  # Most important: meaningful messages
...
def check_branch_structure():
    ...
    score = 0.5 if has_main_branch else 0
    score += 0.5 if has_feature_branches else 0
...
def check_recent_activity():
    ...
    if commit_count == 0:
        return 0, "No recent commits (last 2 weeks)"
    elif commit_count == 1:
